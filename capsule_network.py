...
class CapsuleLayer(nn.Module):
    def __init__(self, num_capsules, num_route_nodes, in_channels, out_channels, kernel_size=None, stride=None,
                 num_iterations=NUM_ROUTING_ITERATIONS, cuda=False):
...
class CapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.primary_capsules = CapsuleLayer(num_capsules=8, num_route_nodes=-1, in_channels=256, out_channels=32,
                                             kernel_size=9, stride=2, cuda=True)
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=32 * 6 * 6, in_channels=8,
                                           out_channels=16, cuda=True)
    ...
    def forward(self, x, y=None):
        ...
        if y is None:
            ...
            if self.cuda:
                y = Variable(torch.sparse.torch.eye(NUM_CLASSES)).cuda().index_select(dim=0,
...
if __name__ == "__main__":
    ...
    def processor(sample, cuda=False):
    ...
    def on_end_epoch(state, cuda=False):
